...
def all_seq_tendsto_filter (x₀ : ℝ) : Filter ℝ :=
  ...
  ⨆ (xi : ℕ → ℝ) (_ : map xi atTop ≤ 𝓝 x₀), Filter.map xi atTop

-- a quick example showing the definition is as intended

example (x₀ : ℝ) (xii : ℕ → ℝ) (h : map xii atTop ≤ 𝓝 x₀) : Filter.map xii atTop ≤ all_seq_tendsto_filter x₀ := by
...
theorem FilterBasedContinuous_iff_FilterBasedTendstoSequence (f : ℝ → ℝ)  (x₀: ℝ):
    ContinuousAt f x₀ ↔ ∀ (xs : ℕ → ℝ), ( FilterBased.TendstoSequence xs x₀) → FilterBased.TendstoSequence (f ∘ xs) (f x₀) := by
      ...
      . intro lt1
        -- define a filter, which is the sets that pass all filters of the form map xs atTop for some xs that tends to x₀
        -- try show that that filter is precisely the sets that pass the filter 𝓝 x₀ (i.e. that contain balls at x0)



        have := lt1 (xi x₀ 1)
        rw [<-Filter.map_map] at this
